...
void Core::server_read_thread( )
{
    try{
        ...
        server_read_thread_started_ = true;
...
// Sends motor orders to the simulator

void Core::server_write_thread( )
{
    try{
        std::cout << "Starting server_write_thread." << std::endl;
...
// Read image_packet and calls manage_received_packet
...
// Reads motor orders from OzCore on the Serial
